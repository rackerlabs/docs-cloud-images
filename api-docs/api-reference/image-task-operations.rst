...
.. note::
   The method and URI (POST tasks) used to create an asynchronous task is the
   same for both importing and export images. However, the operations are
   listed separately to reduce confusion. The desired end result, the request
   and response bodies, and the examples of the two operations are different.
